...
Book::Book( std::string title, std::string author, std::string isbn, double price )
  : _isbn( std::move( isbn ) ), _title( std::move( title ) ), _author( std::move( author ) ), _price( price ) {}
...
Book & Book::operator=( Book && rhs ) & noexcept = default;
...
bool Book::operator==( const Book & rhs ) const noexcept
{
  ...
  return floating_point_is_equal( _price, rhs._price ) && _isbn == rhs._isbn && _title == rhs._title && _author == rhs._author;
}
...
std::istream & operator>>( std::istream & stream, Book & book )
{
  ...
  if( stream >> std::ws >> std::quoted( workingItem._isbn ) && stream >> std::ws >> delimiter && delimiter == ',' && stream >> std::ws >> std::quoted( workingItem._title ) && stream >> std::ws >> delimiter && delimiter == ',' && stream >> std::ws >> std::quoted( workingItem._author ) && stream >> std::ws >> delimiter && delimiter == ',' && stream >> std::ws >> workingItem._price )
  {
